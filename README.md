...
Полнофункциональный Telegram-бот для игроков Escape from Tarkov с **AI-ассистентом «Никита Буянов»** на базе Qwen3-8B. **157 актуальных оружий**, **1965+ модулей**, интеллектуальная генерация сборок, голосовой ввод, конфигуратор оружия, интеграция с tarkov.dev API, полная локализация (RU/EN).

## ✨ Ключевые возможности

### 🤖 AI-ассистент «Никита Буянов» (ОБНОВЛЕНО v5.1)
- **Естественный язык**: Просто напишите, что хотите — "Сделай сборку на АК-74М до 300к"
- **Qwen3-8B локально**: Работает через Ollama без внешних API
- **Умная генерация**: Учитывает актуальные данные, бюджет, совместимость слотов
- **Точные квесты**: Использует реальные требования из tarkov.dev
- **Новости Tarkov**: Спросите "Что нового в Таркове?" — Никита расскажет последние новости
- **Голосовой ввод**: Поддержка голосовых сообщений через Whisper (установка на хосте)
...
- **Динамическая генерация**: Сборки генерируются в реальном времени через API
- **Фильтрация по категориям**: 7 категорий оружия (АР, ПП, СВД, Снайперки и др.)
- **Выбор барахолки**: Только торговцы или с учетом барахолки
- **Неограниченный бюджет**: Возможность пропустить ограничение бюджета
- **Сохранение профиля**: Уровни сохраняются в базе данных
- **Быстрое обновление**: Меняйте уровни в любое время
- **Перегенерация**: Получайте новые варианты с теми же параметрами
...
- **Whisper:** Установка на хост-машине (не в Docker)
- **FFmpeg:** Для конвертации аудио
- **Размер:** ~100-500 MB (модель base/small)

> 💡 **Оптимизация**: Whisper устанавливается локально, Docker образ остаётся легким (~500 MB)
...
# 2. (Опционально) Установите Whisper и Ollama на хосте
pip install openai-whisper  # Для голосовых сообщений
...
> 💡 **Оптимизация**: Docker образ ~500 MB (без ML-библиотек). Whisper и Ollama работают на хосте через volumes.
