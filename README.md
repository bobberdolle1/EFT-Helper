...
Полнофункциональный Telegram-бот для игроков Escape from Tarkov. **700+ единиц оружия**, **6000+ модулей**, готовые мета-сборки, интеграция с tarkov.dev API, полная локализация (RU/EN) и автоматическая загрузка всех данных при первом запуске.
...
- **700+ оружия** с полными характеристиками (калибр, эргономика, отдача)
- **6000+ модулей** (прицелы, приклады, рукоятки, глушители и др.)
- Автоматическая загрузка из tarkov.dev API при первом запуске
- Актуальные цены с Flea Market
- Информация о торговцах и уровнях лояльности
...
- ✅ Создаст базу данных
- ✅ Загрузит 700+ единиц оружия из API
- ✅ Загрузит 6000+ модулей
...
- Создаст базу данных
- Загрузит **700+ оружия** из tarkov.dev API
- Загрузит **6000+ модулей** (прицелы, приклады, рукоятки и т.д.)
- Создаст мета-сборки с полным набором модулей
- Загрузка займет ~1 минуту
...
- **700+ единиц оружия** - полные характеристики (калибр, эргономика, отдача, скорострельность)
- **6000+ модулей** - прицелы, приклады, рукоятки, глушители, магазины, цевья, крепления
- **20+ готовых мета-сборок** - с реальными модулями и ценами
- **11 торговцев** - с эмодзи и уровнями лояльности
- **Автоматическая загрузка** - все данные загружаются из tarkov.dev API
...
2. Загрузит тестовые данные для базовой структуры
3. Загрузит все данные из tarkov.dev API:
   - 700+ единиц оружия с характеристиками
   - 6000+ модулей всех типов
4. Создаст мета-сборки с реальными модулями
5. Запустится

Время первой загрузки: ~1-2 минуты.
...
# Результат: 700+ оружий, 6000+ модулей, 20+ мета-сборок
```
