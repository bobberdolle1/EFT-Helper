...
3. **Выберите пункт [1] - Запустить бота**
...
4. **Выберите пункт [1] - Запустить бота**
...
```bash
./run.sh start      # Запустить бота
./run.sh stop       # Остановить бота
./run.sh restart    # Перезапустить
./run.sh logs       # Показать логи
./run.sh status     # Показать статус
./run.sh rebuild    # Пересобрать образ
./run.sh clean      # Очистить все
./run.sh shell      # Войти в контейнер
```
...
1. **Используйте скрипты** (`run.bat` или `run.sh`) для удобства
